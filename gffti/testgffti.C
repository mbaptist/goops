...
int main()
{
  ...
  n3=n2;
	...
	array<double,1> field(n1);
	array<complex<double>,1> field_hat(n1/2+1);

	FFT< array<double,1> , array<complex<double>,1> > fft;
	
	for (int i=0;i<n1;++i)
		field(i)=sin(2.*M_PI/n1*i);
...
array<double,1> field_hat(n);
	...
	FFT<array<double,1>,array<double,1> > sinfft("sin");
	FFT<array<double,1>,array<double,1> > cosfft("cos");
