...
void v_rfft_h_z_ssc::direct_transform(VCT& u_hat,const VRT& u)
{
  //First two components use sin along z
  ...
  //Third component uses cos along z
  RT s_u(u.shape());
  CT s_u_hat(u_hat.shape());
  for(int i=0;i<s_u.size();++i)
...
void v_rfft_h_z_ssc::inverse_transform(VRT& u,const VCT& u_hat)
{
  //First two components use sin along z
  for(int comp=0;comp<2;++comp)
    {
